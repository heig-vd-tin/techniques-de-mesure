...
\begin{center}
    \fbox{
        \begin{minipage}{0.96\textwidth}
            \textbf{ }
            \begin{enumerate}[a)]
				\item modèle nominal: le rectangle d'incertitude est centré autour des valeurs nominales et contient les valeurs extrêmes mesurées.
				\item modèle réel: comme pour le modèle nominal, mais le rectangle est centré autour des valeurs moyennes; ce rectangle est alors plus petit.
			\end{enumerate}
        \end{minipage}
    }
\end{center}
...
\begin{figure}
    \centering
    \includegraphics[width=12cm]{assets/figures/Ajustage_nominal_gain.PNG}
    \caption{Mesure après ajustage selon le modèle nominal: incertitude sur le gain.}
    \label{fig:Ajustage_nominal_gain}
\end{figure}

En résumé, l'ajustage consiste à :
...
\begin{center}
    \fbox{
        \begin{minipage}{0.96\textwidth}
            \textbf{ }
            \begin{itemize}
                \item Imposer un mesurande $X_{min}$, si possible nul ( $X_0$) et donc correspondant au zéro de mesure, et ajuster le décalage de la chaîne pour obtenir l'offset du modèle nominal ; le zéro de mesure est le point sur lequel le gain n'a pas d'effet.
                ...
                \item Déterminer les incertitudes sur les valeurs ajustées et en déduire les incertitudes d'ajustage:	$D_{c} = \Delta X_{o} + \frac{\Delta Y_{o}}{G} $ et
                      $\alpha _{c} = \frac{D_{c} + \Delta X_{max} + \frac{\Delta Y_{max}}{G}}{X_{max} -X_{o}} $
                      Dans le cas où  $X_{min}$ n'est pas nul, ces relations doivent être adaptées.
            \end{itemize}
        \end{minipage}
    }
\end{center}
